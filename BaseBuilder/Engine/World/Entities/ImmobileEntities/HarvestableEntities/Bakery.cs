...
namespace BaseBuilder.Engine.World.Entities.ImmobileEntities.HarvestableEntities
{
    ...
    public class Bakery : HarvestableEntity
    {
        ...
        static Bakery()
        {
            ...
            {
                new HarvestableRecipe(new List<Tuple<Material, int>> { Tuple.Create(Material.Sugar, 1), Tuple.Create(Material.Egg, 1), Tuple.Create(Material.Wheat, 1) }, new List<Tuple<Material, int>> { Tuple.Create(Material.Bread, 1) })
            };
        }

        protected SpriteRenderer Renderer;

        public Bakery(PointD2D position, int id) : base(position, _CollisionMesh, id, _Recipes)
        {
